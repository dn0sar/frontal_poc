...
with open(log_file_path, "r") as log_file:
    for line in log_file:
        if line[0] == '-':
            ...
            ms_tuples[iter_idx] = (cycles, (int(secret_b1) > 0, int(secret_b2) > 0))
            iter_idx += 1
            ...
            iter_cycles.append(int(line_items[0]))
            secret_b1 = int(line_items[1])
            secret_b2 = int(line_items[2])
...
for inx, comb in enumerate(instr_dict):
    ...
    avgs[inx] = [sum( [ cycles[instr_dict[comb][0]] for cycles, s in ms_tuples if s == comb ] ) / num_occurrences, num_occurrences]
...
for part_num, comb in enumerate(instr_dict):
    ...
    for cy, s in ms_tuples:
        ...
        if s == comb:
            print(f'{cy[instr_dict[comb][0]]}, 1')
